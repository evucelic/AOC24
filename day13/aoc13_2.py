...
for el in t:
    ...
    px = int(el[2].split(":")[1].strip().split(",")[0].strip().split("=")[1].strip()) + 10000000000000
    py = int(el[2].split(":")[1].strip().split(",")[1].strip().split("=")[1].strip()) + 10000000000000

    b = (px*y1 - x1*py) / (x2*y1 - x1*y2)
    if b == int(b) and 0 <= b:
        b = int(b)
        a = (px - x2*b) / x1
        if (a == int(a) and 0 <= a and 
            a*y1 + b*y2 == py):
            a = int(a)
            total += 3*a + b
